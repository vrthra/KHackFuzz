...
class Gif(KaitaiStruct):
    ...
    class Header(KaitaiStruct):
        ...
        def _read(self):
            self.magic = self._io.read_bytes_content(3, b"\x47\x49\x46")
            if not self.magic == b"\x47\x49\x46":
                raise kfuzzstruct.ValidationNotEqualError(b"\x47\x49\x46", self.magic, self._io, u"/types/header/seq/0")
    ...
    class ExtGraphicControl(KaitaiStruct):
        ...
        def _read(self):
            self.block_size = self._io.read_bytes_content(1, b"\x04")
            if not self.block_size == b"\x04":
                raise kfuzzstruct.ValidationNotEqualError(b"\x04", self.block_size, self._io, u"/types/ext_graphic_control/seq/0")
            self.flags = self._io.read_u1()
            self.delay_time = self._io.read_u2le()
            self.transparent_idx = self._io.read_u1()
            self.terminator = self._io.read_bytes_content(1, b"\x00")
            if not self.terminator == b"\x00":
                raise kfuzzstruct.ValidationNotEqualError(b"\x00", self.terminator, self._io, u"/types/ext_graphic_control/seq/4")
    ...
    class ApplicationId(KaitaiStruct):
        ...
        def _read(self):
            self.len_bytes = self._io.read_u1_content(11)
            if not self.len_bytes == 11:
                raise kfuzzstruct.ValidationNotEqualError(11, self.len_bytes, self._io, u"/types/application_id/seq/0")
